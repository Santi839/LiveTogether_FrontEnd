...
export default function VisitorsScreen() {
    ...
    // --- Lógica de Registro (TOTALMENTE CORREGIDA) ---
    const handleRegister = async () => {
        ...
        // ✅ INICIO CORRECCIÓN DE ID: Buscar el ID de usuario de forma robusta
        // Tu login probablemente lo guarda como 'id', pero esta pantalla lo buscaba como 'userId'.
        let userId = await AsyncStorage.getItem('userId');
        if (!userId) {
            console.log("No se encontró 'userId', intentando buscar 'id'...");
            userId = await AsyncStorage.getItem('id'); // Intenta buscar con la llave 'id'
        }
        ...
        if (!userId) {
            Alert.alert(
                ...
                'No se pudo verificar tu usuario. Por favor, inicia sesión de nuevo.'
            );
            return; // No hay ID, no se puede continuar.
        }
        ...
        try {
            ...
            // 1. Se usa 'id_anfitrion' (el campo correcto de Django).
            // 2. Se elimina el campo 'registrado: true' (el backend debe manejarlo).
            const payload: any = {
                nombre_completo: visitorName.trim(),
                tipo_visita: visitType,
                id_anfitrion: parseInt(userId, 10), 
            };

            // Añadir fecha y hora solo si es visitante
            if (visitType === 'visitante' && selectedTime) {
                payload.fecha_visita = selectedDateString;
                payload.hora_llegada = `${selectedTime}:00`; 
            } else {
                payload.fecha_visita = null;
                payload.hora_llegada = null;
            }
            ...
            console.error('Error al registrar visitante:', error.response?.data || error.message);
